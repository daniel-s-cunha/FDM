...
# def plot_system_dynamics(histories, deliveries, m, k=10, sim_lw=0.75, sim_ls='-', sim_color='gray', sim_alpha=0.4):
#     """
#     Plots the median, the (0.25, 0.75) quantiles, and k individual simulations for all requested variables.
#     """
...
#     # Randomly select k simulation indices to plot (ensures the same paths are plotted across all subplots)
#     n_samples = deliveries.shape[1]
#     np.random.seed(42) # Optional: keeps the chosen simulations consistent across replots
#     sim_indices = np.random.choice(n_samples, size=min(k, n_samples), replace=False)
...
#         # --- NEW: Plot the individual simulation paths in the background ---
#         # Matplotlib handles 2D arrays by plotting each column as a separate line
#         ax.plot(months, data[:, sim_indices], color=sim_color, 
#                 linewidth=sim_lw, linestyle=sim_ls, alpha=sim_alpha)
...
def plot_system_dynamics(histories, deliveries, m, k=10, sim_lw=0.75, sim_ls='-', sim_color='gray', sim_alpha=0.4):
    ...
    for ax, (ylabel, data, color) in zip(axes, variables):
        ...
        # --- PLOT SIMULATIONS FIRST (zorder=1) ---
        ax.plot(months, data[:, sim_indices], color=sim_color, 
                linewidth=sim_lw, linestyle=sim_ls, alpha=sim_alpha, zorder=1)
        ...
        if ct == 6 and run_out_month is not None:
            ...
            # Plot the segment before and up to the run out point in blue
            ax.plot(months[:idx+1], median_val[:idx+1], color='blue', linewidth=linewidth_val,linestyle=linestyle_val, zorder=3)
            ax.fill_between(months[:idx+1], q25[:idx+1], q75[:idx+1], color='blue', alpha=alpha_val, zorder=2)
            
            # Plot the segment from the run out point onward in red
            ax.plot(months[idx:], median_val[idx:], color='red', linewidth=linewidth_val2,linestyle=linestyle_val, zorder=3)
            ax.fill_between(months[idx:], q25[idx:], q75[idx:], color='red', alpha=alpha_val2, zorder=2)
            
        else:
            # Plot all other subplots normally in blue
            ax.plot(months, median_val, color='blue', linewidth=linewidth_val,linestyle=linestyle_val, zorder=3)
            ax.fill_between(months, q25, q75, color='blue', alpha=alpha_val, zorder=2)
...
plot_system_dynamics(histories, optimal_deliveries, m, k=15, sim_lw=1.25, sim_ls='-', sim_color='black', sim_alpha=0.2)